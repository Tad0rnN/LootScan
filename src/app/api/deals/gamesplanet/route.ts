...
import { getGamesplanetDeals } from "@/lib/gamesplanet-deals";
import type { GamesplanetRegion } from "@/lib/sources/gamesplanet-feed";

const VALID_REGIONS = new Set(["fr", "uk", "de", "us"]);

function isGamesplanetRegion(value: string | null): value is GamesplanetRegion {
...
export async function GET(req: NextRequest) {
  ...
  try {
    const deals = await getGamesplanetDeals({
      region: isGamesplanetRegion(region) ? region : undefined,
    ...
    console.error("Gamesplanet deals API error:", err);
